...
const (
	...
	userAgent      = "ip-netblocks-go/" + libraryVersion
	mediaType      = "application/json"
)

// defaultIPNetblocksURL is the default IP Netblocks API URL.
const defaultIPNetblocksURL = `https://ip-netblocks.whoisxmlapi.com/api/v2`
...
type ClientParams struct {
	...
	// IPNetblocksBaseURL is the endpoint for 'IP Netblocks API' service
	IPNetblocksBaseURL *url.URL
}
...
func NewClient(apiKey string, params ClientParams) *Client {
	...
	apiBaseURL := params.IPNetblocksBaseURL
	if apiBaseURL == nil {
		apiBaseURL, err = url.Parse(defaultIPNetblocksURL)
		if err != nil {
			panic(err)
		}
	}
	...
	client.IPNetblocks = &ipNetblocksServiceOp{client: client, baseURL: apiBaseURL}

	return client
}

// Client is the client for IP Netblocks API services.
type Client struct {
	client *http.Client
